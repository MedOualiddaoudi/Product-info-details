...
def create_packs_from_json(json_data, graph):
    """Create nodes and relationships in the Neo4j graph based on the PACK JSON data."""
    for pack in json_data:
        # Pack node setup
        pack_node = Node("Pack", productId=pack["productId"], productType=pack["productType"], productName=pack["productName"])
        graph.merge(pack_node, "Pack", "productId")

        # Target nodes setup
        for target_desc in pack["productTarget"]:
            target = Node("Target", description=target_desc)
            graph.merge(target, "Target", "description")
            rel = Relationship(pack_node, "HAS_TARGET", target)
            graph.merge(rel)

        # Formula nodes setup
        for formula in pack["productDetails"]["formulas"]:
            ...
            rel = Relationship(pack_node, "HAS_FORMULA", formula_node)
            graph.merge(rel)
...
json_data = load_json_file(r"D:\2ia projects\Stage 2a\DATASET\Pack\sample_pack_products.json")
create_packs_from_json(json_data, graph)
